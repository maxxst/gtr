 ...
 * acted upon). This obviously includes things like the player, NPCs, and
 * monsters. Not as obviously, an {@code Actor} can represent things like items,
 * spells, doors and traps, or even abstract actions like timers and events. An
 * {@code Actor} can be attached to one another in order to create other
 * interesting effects. For example, an enchantment spell might be an effect
 * {@code Actor} attached to an item {@code Actor}. A timer {@code Actor} could
 * be attached to the spell which simple counts an appropriate amount of turns,
 * then expires the spell {@code Actor}.
 */
public abstract class Actor extends Messenger {
	...
	/**
	 * Performs the actions of this {@code Actor}. Normally this would be called
	 * by the {@code World} in the {@code tick()} method, which allows the
	 * {@code Actor} actions to be scheduled properly based on act order and
	 * {@code Actor} speed.
	 */
	...
	/**
	 * Returns the {@code World} the {@code Actor} is currently bound to, or
	 * null if the {@code Actor} is unbound.
	 * 
	...
	/**
	 * Returns true if the {@code Actor} bound to a {@code World}. This is
	 * equivalent to calling {@code actor.world() != null}.
	 * 
	...
	/**
	 * Returns true if the {@code Actor} is bound to the given {@code World}.
	 * This is equivalent to calling {@code actor.world() == world}.
	 * 
	 * @param world
	 *            the {@code World} being tested
	...
	/**
	 * Sets the position of the {@code Actor} on its current {@code World}. The
	 * {@code Actor} must be bound and not held when this method is called.
	 * 
	 * @param x
	 *            the new x position of the {@code Actor}
	 * @param y
	 *            the new y position of the {@code Actor}
	 */
	 ...
	 * 
	 * @param coord
	 *            the new position of the {@code Actor}
	 */
	...
	public final void movecare(int dx, int dy) {
		try {
			if (world().passableAt(x() + dx, y() + dy)
					&& world().getActorAt(Creature.class, x() + dx, y() + dy) == null) {
				move(dx, dy);
			} else {
	...
	public final void speakTo(Direction dir) {
		Human human = world().getActorAt(Human.class, x() + dir.dx(),
				y() + dir.dy());
		if (human == null)
			...
			try {
				...
				while (i < human.getMessages().size()) {
					char key = player.getTerm().getKey();
					switch (key) {
					case Key.speakToPerson:
						displayText(human.getMessages().get(i));
						i++;
						break;
					case Key.skipAllMessages:
						i = human.getMessages().size();
						eventText("Du hast das Gespräch abgebrochen.");
						break;
					}
				}
	...
	/**
	 * Returns true if the {@code Actor} is expired, or in other words, marked
	 * for removal.
	 * 
	...
	/**
	 * Attaches this {@code Actor} to another. Note that the {@code Actor} must
	 * not be currently held or bound to a {@code World} when calling this
	 * method. The position of this {@code Actor} will follow the holder, and
	 * the {@code Actor} will be bound to the holder's {@code World}. However,
	 * the {@code Actor} will not be placed on the {@code World} grid and will
	 * therefore not be accessible through {@code world.getActorAt()} or
	 * {@code world.getActorsAt()}.
	 * 
	 * @param holder
	 *            the new holder of the {@code Actor}
	 */
	...
	/**
	 * Detaches this {@code Actor} from its current holder. Note that the
	 * {@code Actor} must currently be held when calling this method. If the
	 * {@code Actor} is bound, it will be placed on the {@code World} at the
	...
	/**
	 * Returns the current holder of the {@code Actor}, or null if it is not
	 * held.
	 * 
	...
	/**
	 * Returns true if the {@code Actor} is currently held. This is equivalent
	 * to {@code actor.holder() != null}.
	 * 
	...
	/**
	 * Returns true if the {@code Actor} is held by the specified {@code Actor}.
	 * This is equivalent to {@code actor.holder() == holder}.
	 * 
	 * @param holder
	 *            the holder being tested
	...
	/**
	 * Returns all the {@code Actor} the specified class currently being held by
	 * this one.
	 * 
	 * @param <T>
	 *            the generic type of the class being queried
	 * @param cls
